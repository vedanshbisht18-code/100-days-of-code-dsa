...
void dfs(int u, struct Node* adj[], int visited[]) {
    visited[u] = 1;
    for (struct Node* p = adj[u]; p != NULL; p = p->next) if (!visited[p->v]) dfs(p->v, adj, visited);
}
...
int main(void) {
    ...
    adj[2] = add_edge(adj[2], 3);
    adj[3] = add_edge(adj[3], 2);
    adj[3] = add_edge(adj[3], 4);
    adj[4] = add_edge(adj[4], 3);
    int visited[5] = {0};
    dfs(1, adj, visited);
    int connected = 1;
    for (int i = 1; i <= n; ++i) if (!visited[i]) connected = 0;
    printf("Connected: %s\n", connected ? "yes" : "no");
    return 0;
}